...
class Test(unittest.TestCase):
    ...
    def runOnce(self):
        ...
        for video in Test.videos:
            ...
            video['kafy'] = kafy.new(video['identifier'])
            video['streams'] = video['kafy'].streams
            video['best'] = video['kafy'].getbest()
            video['bestaudio'] = video['kafy'].getbestaudio()
            video['bestvideo'] = video['kafy'].getbestvideo()

            # get urls for age restricted vids
            if video['kafy'].videoid == "07FYdnEawAQ":
                _ = video['kafy'].streams[0].url
                _ = video['kafy'].streams[1].url_https
                del _

        for playlist in Test.playlists:


            playlist['fetched'] = kafy.get_playlist(playlist['identifier'])
    ...
    def test_generate_filename_with_meta(self):
        ...
        p = kafy.new('jrNLsC_Y9Oo', size=True)
        a = p.getbestaudio()
    ...
    def test_kafy_download_resume(self):
        ...
        with open(tempname, "w") as ladeeda:
            ladeeda.write("abc")
        vid = kafy.new("DsAn_n6O5Ns", gdata=True, basic=False)
    ...
    @stdout_to_null
    def test_kafy_download_to_dir(self):
    ...
    #def test_lazy_kafy(self):
    #    """ Test create kafy object without fetching data. """

    #    vid = kafy.new("DsAn_n6O5Ns", basic=False, signature=False)
    ...
    def test_kafy_init(self):
        """ Test kafy object creation. """
        # test bad video id, 11 chars
        badid = "12345678901"
        too_short = "123"
        self.assertRaises(ValueError, kafy.new, too_short)
        self.assertRaises(ValueError, kafy.get_playlist, badid)
        self.assertRaises(IOError, kafy.new, badid)
        self.assertRaises(IOError, kafy.get_playlist, 'PL' + "a" * 16)

    def test_video_properties(self):
        """ Test video properties. """
        for video in Test.videos:
            description = video['kafy'].description.encode("utf8")
            ...
            for prop in self.properties:
                ...
                if prop != "thumb":
                    paf_prop = getattr(video['kafy'], prop)
                    exp_prop = video[prop]
