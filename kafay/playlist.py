...
from . import g
from .kafy import new, get_categoryname, call_gdata, fetch_decode
...
def get_playlist(playlist_url, basic=False, gdata=False,
    ...
    for v in allinfo['video']:
        ...
        try:
            kafy_obj = new(vid_data['encrypted_id'],
                           basic=basic,
                           gdata=gdata,
                           size=size,
                           callback=callback)
        ...
        kafy_obj.populate_from_playlist(vid_data)
        playlist['items'].append(dict(kafy=kafy_obj,
